...
export class TokenService {
  static async reserveTokens(
    ...
    try {
      const { error } = await supabase.rpc<TokenRPCParams, null>('reserve_tokens', {
        p_user_id: userId,
  ...
  static async releaseTokens(
    ...
    try {
      const { error } = await supabase.rpc<TokenRPCParams, null>('release_tokens', {
        p_user_id: userId,
        p_property_id: propertyId,
        p_token_count: quantity
      });
  ...
  static async transferTokens(
    ...
    try {
      const { error } = await supabase.rpc<TransferTokensParams, null>('transfer_tokens', {
